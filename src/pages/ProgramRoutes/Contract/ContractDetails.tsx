...
const ContractDetails: React.FC = () => {
  ...
  async function fetchJourneyTypeById(id: string): Promise<JourneyType> {
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/api/journey/getJourney/${id}`
    );
  ...
  async function fetchLuggageById(id: string): Promise<LuggageType> {
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/api/luggage/getLuggage/${id}`
    );
    const data = await response.json();
    return data;
  }
  async function fetchVehicleTypeById(id: string): Promise<VehicleType> {
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/api/vehicleType/getVehiclesType/${id}`
    );
    const data = await response.json();
    return data;
  }
